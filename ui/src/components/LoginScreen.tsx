    ...
    try {
      ...
      if (credentials.party === "foundation") {
        ...
        if (userContract === null) {
          ...
          userContract = await ledger.create(Portfolio.FoundationUser, user);
        }
        onLogin(credentials);
      } else {
        let userContract = await ledger.fetchByKey(Portfolio.Customer, credentials.party);
        if (userContract === null) {
          const user = {username: credentials.party, following: []};
          userContract = await ledger.create(Portfolio.Customer, user);
        }
        onLogin(credentials);
      }
    } catch(error) {
